...
def log_quiz_result(profile: dict, subject: str, topic: str, is_correct: bool) -> dict:
    """
    Logs a quiz result to the full user profile, updating both the log and the aggregated data.
    """
    # 1. Create the new log entry and add it to the performance_log list
    log_entry = {
    ...
    # Use .setdefault to create the list if it doesn't exist, preventing KeyErrors
    profile.setdefault("performance_log", []).append(log_entry)

    # 2. Access the nested performance data and update it
    # .setdefault is great here for safely creating nested dictionaries if they're new
    subject_data = profile.setdefault('performance_data_per_subject', {}).setdefault(subject, {})
    topic_data = subject_data.setdefault(topic, {'correct': 0, 'total': 0})
